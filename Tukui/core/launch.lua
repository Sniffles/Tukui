...
local function chatsetup()
	...
	for i = 1, NUM_CHAT_WINDOWS do
		...
		frame:Size(370 + 1, 111)
		
		-- tell wow that we are using new size
		SetChatWindowSavedDimensions(chatFrameId, T.Scale(370 + 1), T.Scale(111))
		
		-- move general bottom left or Loot (if found) on right
		if i == 1 then
			frame:ClearAllPoints()
			frame:Point("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 2, 2)
		end
		...
		if i == 1 then FCF_SetWindowName(frame, "A") end
		if i == 2 then FCF_SetWindowName(frame, "L") end
	end
